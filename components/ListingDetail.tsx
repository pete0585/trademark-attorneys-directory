...
export default function ListingDetail({ listing }: Props) {
    ...
    return (<div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
        ...
        <span>/</span>
        <span className="text-gray-900">{(listing.full_name || listing.name || 'Attorney')}</span>
      </div>
      ...
      <div className={`card p-6 mb-6 ${isFeatured ? 'border-brand-amber/40' : ''}`}>
        <div className="flex flex-col sm:flex-row gap-5">
          {listing.photo_url ? (<Image src={listing.photo_url} alt={(listing.full_name || listing.name || 'Attorney')} width={96} height={96} className="w-24 h-24 rounded-xl object-cover shrink-0"/>) : (<div className="w-24 h-24 rounded-xl bg-brand-indigo/10 flex items-center justify-center shrink-0">
              <span className="text-3xl font-bold text-brand-indigo">
                {(listing.full_name || listing.name || 'Attorney').charAt(0)}
              </span>
            </div>)}
          ...
          <div className="flex-1">
            ...
            <h1 className="text-2xl font-bold text-gray-900">{(listing.full_name || listing.name || 'Attorney')}</h1>
            {listing.firm_name && (<p className="text-gray-600 mt-0.5">{listing.firm_name}</p>)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {(listing.specialties ?? []).length > 0 && (<div className="card p-5">
              <h2 className="font-semibold text-gray-900 mb-3">Specializes in Clients Who Are...</h2>
              <div className="flex flex-wrap gap-2">
                {(listing.specialties ?? []).map((ct) => (<Link key={ct} href={`/categories/${ct}`} className="flex items-center gap-1.5 text-sm bg-brand-amber/5 hover:bg-brand-amber/10 text-brand-amber-dark px-3 py-1.5 rounded-lg font-medium transition-colors">
          ...
          {!listing.claimed_at && (<div className="card p-5 border-dashed">
              <p className="text-sm text-gray-500 mb-3">
                Are you <strong>{(listing.full_name || listing.name || 'Attorney')}</strong>? Claim this listing to add your photo, bio, specializations, and contact form.
              </p>
        ...
        <div className="space-y-4">
          <div className="card p-5">
            ...
            {(listing.phone || listing.email || listing.website) ? (<div className="space-y-3">
                {listing.phone && (<a href={`tel:${listing.phone}`} className="flex items-center gap-2 text-sm text-gray-700 hover:text-brand-indigo transition-colors">
                    <Phone className="w-4 h-4 text-brand-indigo shrink-0" aria-label="Phone"/>
                    {formatPhone(listing.phone)}
                  </a>)}
                {listing.email && (<a href={`mailto:${listing.email}`} className="flex items-center gap-2 text-sm text-gray-700 hover:text-brand-indigo transition-colors">
                    <Mail className="w-4 h-4 text-brand-indigo shrink-0" aria-label="Email"/>
                    {listing.email}
                  </a>)}
                {listing.website && (<a href={listing.website.startsWith('http') ? listing.website : `https://${listing.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-gray-700 hover:text-brand-indigo transition-colors">
                    <Globe className="w-4 h-4 text-brand-indigo shrink-0" aria-label="Website"/>
                    Visit Website
                  </a>)}
              </div>) : (<p className="text-sm text-gray-400">Contact info not yet provided. <Link href={`/claim/${listing.id}`} className="text-brand-indigo hover:underline">Claim this listing</Link> to add it.</p>)}

            {listing.plan_tier === 'free' && (<></>)}
          </div>

          {(listing.languages ?? []).length > 0 && (<div className="card p-4">
              <h3 className="font-medium text-gray-900 text-sm mb-2">Languages</h3>
              <div className="flex flex-wrap gap-1.5">
                {(listing.languages ?? []).map((lang) => (<span key={lang} className="text-xs bg-surface text-gray-600 px-2 py-0.5 rounded">
                    {lang}
                  </span>))}
              </div>
            </div>)}
