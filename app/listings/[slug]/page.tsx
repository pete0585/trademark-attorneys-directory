...
import { MapPin, Phone, Globe, Mail, CheckCircle, Star, Scale, ArrowLeft, Users, BookOpen } from 'lucide-react';
...
export default async function ListingDetailPage({ params }: Props) {
    ...
    return (<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <Link href="/listings" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-navy-600 mb-6 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <img src={attorney.photo_url} alt={displayName} className="w-16 h-16 rounded-full object-cover"/>) : (<span className="text-navy-600 font-bold text-2xl">
              ...
              <div className="flex-1">
                ...
                {attorney.firm_name && attorney.full_name !== attorney.firm_name && (<p className="text-slate-600 mt-0.5">{attorney.full_name}</p>)}
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  ...
                  {null}
                  {attorney.accepting_new_clients && (<div className="flex items-center gap-1 text-emerald-600 text-sm">
            ...
            {attorney.bio && (<div className="mt-5 pt-5 border-t border-slate-100">
                <h2 className="font-semibold text-navy-800 mb-2">About</h2>
                <p className="text-slate-600 text-sm leading-relaxed">{attorney.bio}</p>
              </div>)}

            {attorney.specialties && attorney.specialties.length > 0 && (<div className="mt-5">
                <h2 className="font-semibold text-navy-800 mb-2">Specialties</h2>
                <div className="flex flex-wrap gap-2">
                  {attorney.specialties.map((s) => (<Link key={s} href={`/listings?specialty=${encodeURIComponent(s)}`} className="text-sm bg-navy-50 text-navy-700 px-3 py-1 rounded-full border border-navy-100 hover:bg-navy-100 transition-colors">
                      {s}
                    </Link>))}
                </div>
              </div>)}

            {attorney.areas_of_practice && attorney.areas_of_practice.length > 0 && (<div className="mt-4">
                <h2 className="font-semibold text-navy-800 mb-2">Practice Areas</h2>
                <div className="flex flex-wrap gap-2">
                  {attorney.areas_of_practice.map((a) => (<span key={a} className="text-sm bg-slate-50 text-slate-600 px-3 py-1 rounded-full border border-slate-200">
                      {a}
                    </span>))}
                </div>
              </div>)}
          ...
          {/* Bar info */}
          {attorney.bar_number && (<div className="bg-white rounded-2xl border border-slate-200 shadow-soft p-6">
          ...
          {/* Languages */}
          {attorney.languages && attorney.languages.length > 1 && (<div className="bg-white rounded-2xl border border-slate-200 shadow-soft p-6">
              <h2 className="font-semibold text-navy-800 mb-3">Languages</h2>
              <div className="flex flex-wrap gap-2">
                {attorney.languages.map((l) => (<span key={l} className="text-sm bg-slate-50 text-slate-600 px-3 py-1 rounded-full border border-slate-200">
                    {l}
                  </span>))}
              </div>
            </div>)}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-soft p-5 sticky top-20">
            ...
            <div className="space-y-3 text-sm">
              {attorney.address && (<div className="flex gap-3 text-slate-600">
              ...
              {attorney.phone && (<a href={`tel:${attorney.phone}`} className="flex items-center gap-3 text-navy-600 hover:text-navy-500 transition-colors">
                  <Phone className="w-4 h-4 flex-shrink-0"/>
                  {formatPhone(attorney.phone)}
                </a>)}
              {attorney.email && (<a href={`mailto:${attorney.email}`} className="flex items-center gap-3 text-navy-600 hover:text-navy-500 transition-colors">
                  <Mail className="w-4 h-4 flex-shrink-0"/>
                  <span className="truncate">{attorney.email}</span>
                </a>)}
              {attorney.website && (<a href={attorney.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-navy-600 hover:text-navy-500 transition-colors">
            ...
            {attorney.website && (<a href={attorney.website} target="_blank" rel="noopener noreferrer" className="mt-4 block w-full text-center bg-navy-600 hover:bg-navy-500 text-white font-semibold py-2.5 rounded-xl transition-colors text-sm">
                Visit Website
              </a>)}

            {!attorney.claimed && (<Link href={`/claim?slug=${attorney.slug}`} className="mt-2 block w-full text-center border border-gold-400 text-gold-700 hover:bg-gold-50 font-medium py-2.5 rounded-xl transition-colors text-sm">
      ...
      <div className="mt-8 rounded-xl bg-gray-50 border border-gray-200 p-5">
        <p className="text-sm text-gray-600">
          ...
          <a href={`/claim/${attorney.id}`} className="underline hover:opacity-80">Claim your free listing</a>
