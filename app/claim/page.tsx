...
const TIERS = [
    ...
    {
        name: 'Verified',
        price: "Free listing \u2014 public contact details included",
        features: [
            'Everything in Basic',
            'Verified badge',
            'Bio & photo',
...
export default function ClaimPage() {
    return (<div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12">
        {TIERS.map((tier) => (<div key={tier.name} className={`bg-white rounded-2xl border p-6 ${tier.primary
                ? 'border-navy-400 ring-2 ring-navy-200 shadow-card'
                : 'border-slate-200 shadow-soft'}`}>
            {tier.note && (<div className="text-xs font-semibold text-navy-600 bg-navy-50 rounded-full px-3 py-0.5 inline-block mb-3">
                {tier.note}
              </div>)}
            ...
            <ul className="space-y-2 mb-6">
              {tier.features.map((f) => (<li key={f} className="flex items-start gap-2 text-sm text-slate-600">
                  <CheckCircle className="w-4 h-4 text-gold-500 flex-shrink-0 mt-0.5"/>
                  {f}
                </li>))}
            </ul>
            <a href={`mailto:hello@trademarksearchdirectory.com?subject=${encodeURIComponent(`${tier.name} Listing Claim`)}`} className={`block w-full text-center py-2.5 rounded-xl font-semibold text-sm transition-colors ${tier.primary
      ...
      <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6 md:p-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <a href="mailto:hello@trademarksearchdirectory.com" className="inline-flex items-center gap-2 bg-navy-600 hover:bg-navy-500 text-white font-semibold px-5 py-2.5 rounded-xl text-sm transition-colors">
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
        ...
        ].map(({ icon: Icon, title, desc }) => (<div key={title} className="bg-white rounded-xl border border-slate-200 p-4">
